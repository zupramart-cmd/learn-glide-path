...
export default function AdminAddVideoPage() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "videos", editId)).then((snap) => {
      if (snap.exists()) {
        ...
        setEditVideo(v);
        setCourseId(v.courseId); setSubjectId(v.subjectId); setChapterId(v.chapterId || "");
        setTitle(v.title); setThumbnail(v.thumbnail); setVideoURL(v.videoURL); setPdfURL(v.pdfURL);
      } else {
  ...
  if (loading) return <FormPageSkeleton sections={1} fieldsPerSection={6} />;

  return (
    <div
      className="animate-fade-in w-full max-w-2xl mx-auto overflow-x-hidden overflow-y-auto pb-8"
    ...
    >
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-4 py-3 flex items-center gap-2">
        <button onClick={() => navigate("/admin/videos")} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <form onSubmit={handleSubmit} className="px-3 sm:px-4 pt-4 space-y-4">
        ...
        >
          {success ? (<><CheckCircle className="h-4 w-4" /> Added!</>)
            : submitting ? (editVideo ? "Updating..." : "Adding...")
