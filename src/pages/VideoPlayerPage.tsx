...
export default function VideoPlayerPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const allVids = await getCachedCollection<Video>(
          db,
          "videos",
        ...
        if (cancelled) return;
        const vids = allVids.filter((vid) => vid.subjectId === v.subjectId);
  ...
  return (
    <div
      className={`animate-fade-in lg:flex lg:gap-4 lg:p-4 ${
        isMobile
          ? "flex flex-col overflow-hidden h-[calc(100vh-7rem)]"
          : "h-[calc(100vh-3.5rem)]"
      }`}
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="max-h-60 overflow-y-auto min-w-[180px]">
        ...
        {chapters.map((ch) => (
          <DropdownMenuItem
            key={ch.chapterId}
            onClick={() => onChange(ch.chapterName)}
            className="flex items-center justify-between gap-2"
          >
